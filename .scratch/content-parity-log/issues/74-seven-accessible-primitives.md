...
Type: task
Status: resolved 2026-08-13 — built in commit `34a9e96`; the status line was never moved
off `ready-for-agent`. See the answer, which is a verification and not a build record.
...
**Resolved 2026-08-13 by verification, not by building.** The work landed in commit
`34a9e96` — *Seven primitives arrive, and the tab strip answers an arrow key* — and the
`Status:` line was never moved. A triage sweep of every open ticket found it.

**This answer is a code read, not a hands-on check of the running interface.** The
keyboard behaviour this ticket exists for was verified by a human at build time, per the
commit; nothing here re-tests it.

What is in the tree:

- **All seven primitives** at `web/src/components/ui/` — `dialog`, `popover`, `tooltip`,
  `select`, `checkbox`, `tabs`, `table`. Later tickets have added more beside them.
- **`@base-ui/react ^1.7.0`** in `web/package.json`, with `shadcn ^4.17.0` as a dev
  dependency. No TypeScript: `web/jsconfig.json` carries the `@/*` → `./src/*` alias and
  there is no `web/tsconfig.json`.
- **The tab strip is the first user**, as the ticket asked — `web/src/components/Ledger.jsx`
  lines 15 and 221, with the strip at 243-262.
- **The palette still wins.** The precedence is written down rather than left to import
  order: `web/src/styles/app.css` lines 112, 122 and 217-235. The commit message records
  that `palette.mjs`, `Chips.jsx` and `Diff.jsx` were deliberately not rebuilt, which is
  what this ticket's *a palette that keeps meaning* half required.
