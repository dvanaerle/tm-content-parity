...
Blocked by: None — 103, 102 and 123 are all resolved. This can start immediately.
Parent: ../map.md
...
> shape — **a commit per area, not a ticket per area.**
>
> The filename keeps 104's slug so every inbound link in `map.md` and in the sibling
> tickets still resolves. The five parts below are **A** (what nothing means), **B** (the
> notes), **C** (the chip), **D** (the suggestions) and **E** (the page row); each is one
> commit, in that order. A depends on nothing, B and D and E depend on A's classification
> being a value, and C is independent of all of them.

**What to build:** page scope, which 103 taught the search box to parse, becomes a feature
an editor can actually find and actually trust. Today it is invisible punctuation that
returns a blank.

## Before the first commit: the labels are English now

Every part below names Dutch labels — *Eenzijdige pagina's*, *Pagina's*, *Verschillen*,
*filter wissen*, *inclusief afgesloten*. **All five tickets were written before 124
landed**, and 124 made the interface English (UK) on all six stores and renamed
*Verschillen* to **Repeats**. Read the label names below as pointers to surfaces, not as
strings to type, and take the current name from `web/src` and from `CONTEXT.md`, which
wins. The stopword guard 124 installed will catch a slip; do not excuse it.

---

## A — A scoped search says which kind of nothing it found

An editor scopes to a page and gets nothing back, and the screen tells them **which**
nothing it is. There are four, and today they are one blank:

- **No such page.** The scope matches no key. A typo, and the answer is to try again.
- **The page has one side.** It exists, it is in the store, and it is in the one-sided
  pages aside — but one side did not answer, so it is not compared and it is not in the
  index. Search returning silence here is search contradicting the page list on the same
  screen.
...
- [ ] The clear-filter control clears the scope and the classes together, and the search
      box loses the scope while keeping any remaining term.
...
An editor types `/` and the box offers the store's page keys, narrowing as they keep
typing. Without this, page scope is a feature only someone who has read the source can use.
...
An editor looking at the pages table finds the page they care about, and gets from there
into a scoped search without typing an opaque key by hand.
...
trades a budgeted wall for an unbudgeted one, and it demotes the queue 81 established. What
the ask was actually reaching for is this — a way to go from *this page* to *what is on this
page* — and it is a click, not a default.
...
**On the four kinds of nothing (A)**
...
- Ticket 83 adds page-scope notes with a priority. It needs no key-shape change and this
  ticket should not pre-empt its vocabulary — only make room for a second kind of note to be
  told apart.
...
- Scoping from a one-sided page is reachable from the one-sided pages aside too, and lands
  on **A**'s explanation rather than on silence.
