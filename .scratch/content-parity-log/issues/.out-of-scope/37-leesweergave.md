...
before specifying it again.

## Parked — 2026-08-11

> *This was generated by AI during triage.*

**The decision is deferred, not made.** Every criterion in this ticket was met and
every test passed, so there is nothing here to fix and nothing to argue with. What
is unsettled is whether a second reading of the same page earns the surface it
costs — a product question, to be taken up in a brainstorm.

**Re-open trigger.** Any one of these:

- An editor asks to judge a page as a reader rather than as a table of rows — the
  need this ticket was written for, stated by a user rather than inferred.
- The `alt-lost` distinction is lost in practice: production carries 50 images with
  **no alt attribute** and the new site always writes the attribute, and the current
  Markdown renders both as `![](file.jpg)`. If that costs someone real time, the
  reading view is the artefact that fixes it.
- Somebody wants a jump list on real `h1`–`h6` anchors. Point 3 below explains why
  this view is where that works best.

**The build is preserved on the branch `park/ticket-37-leesweergave`, not on a stash.**
The original stash was promoted to a branch on 2026-08-11 so the work cannot be lost
to `git stash drop` or a reflog expiry while the ticket is parked.

**Restore it with `git stash apply park/ticket-37-leesweergave`, not with
`git checkout`.** The branch tip is a stash-shaped commit and the five *new* files
live on its third parent, because the stash was taken with untracked files included.
A plain `git checkout` of the branch silently gives you the four modified files and
none of the new ones:

| where | files |
| --- | --- |
| the commit's own tree | `compare/contract.mjs`, `crawl/extract.mjs`, `crawl/extract.test.mjs`, `web/src/components/ContentView.jsx` |
| `park/ticket-37-leesweergave^3` | `shared/markdown.mjs`, `shared/markdown.test.mjs`, `web/src/lib/reading.mjs`, `web/src/lib/reading.test.mjs`, `web/src/components/Leesweergave.jsx` |

Nine files, not the six this ticket's prose counts. The branch is local and has not
been pushed.
...
The build was on the git stash `ticket 37 Leesweergave: built 2026-08-11, reverted
pending a value decision`, now also reachable as the branch `park/ticket-37-leesweergave`
— see *Parked* above for the restore command. It is worth ten minutes of a reader's
time before anyone specifies this again.
