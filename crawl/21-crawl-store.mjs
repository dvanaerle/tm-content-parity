...
const FAILURES = new URL('../data/extract-failures.json', import.meta.url);
...
await writeFile(FAILURES, JSON.stringify({ store, at: new Date().toISOString(), failures }, null, 2));
...
if (failures.length > 20) console.log(`  … and ${failures.length - 20} more, in ${fileURLToPath(FAILURES)}`);
