...
(For reference, a multi-context repo — signalled by a `CONTEXT-MAP.md` at the root — instead points at one `CONTEXT.md` per context, typically under `src/<context>/`, each with its own `docs/adr/`.)
