 ...
 * `view.test.mjs`, and what is only visible here is what the screen does with them — that the
 * class label is a link and not a control, and which rows carry a tick (ticket 04).
 *
 * **Whether a row names its store is not here** (ADR 0030). It is the reading's answer, and
 * `list-reading.test.mjs` asks it directly; a browser case duplicating a unit test is what
 * makes the next reader distrust both.
 *
...
async function mount(search = "") {
  ...
  await act(async () =>
    root.render(createElement(AllStores, { stores: STORES })),
  );
...
const rows = () =>
  [...document.querySelectorAll('[data-row="difference"]')].map((row) =>
...
const classLabels = () => [
  ...document.querySelectorAll('a[data-badge="class"]'),
];
...
describe("a class label opens a queue over every store", () => {
  ...
  it("ticks only what may be pressed, where the two kinds of row are on one list", async () => {
    ...
    expect(
      selectLabels().filter((label) => label.startsWith("Select afhalen on")),
    ).toHaveLength(6);
